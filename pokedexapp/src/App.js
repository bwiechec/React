  ...
  render() {
          ...
          <TableRow key={number} style={{backgroundColor: "white"}}>
            ...
            <TableCell key={'TableCell_Link_' + number} align="center">
              <NavLink to={`/pokemon/${number}`}
                       key={number + '_link'}>
                More
                about {this.state.pokemons[key]['name'][0].toUpperCase() + this.state.pokemons[key]['name'].slice(1,)}
              </NavLink>
            </TableCell>
          </TableRow>
        )
    ...
    return (
      <div key={'App_Div'} className="App" style={{display: "flex", flexDirection: "row", alignItems: "flex-start"}}>
        <div key={'TableDiv'} className={"sticky"}
             style={{display: "block", padding: '1rem', paddingBottom: '0.1rem', top: "0", minHeight: "90%"}}>
          {!this.state.loading ?
